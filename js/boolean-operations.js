...
var text = new PointText({
    position: view.center + new Point(0, 200) * scaleFactor,
    fillColor: 'black',
    justification: 'center',
    fontSize: 20 * scaleFactor * 2
});
...
function onFrame(event) {
    if (activeItem != ring) {
        ...
        var offset = new Point(140, 80) * [scaleFactor * Math.sin(event.count / 60), scaleFactor * Math.sin(event.count / 40)];
...
function onResize() {
    text.position = view.center + new Point(0, 200) * scaleFactor;
    square.position = view.center;
}